...
namespace CCPoseLoader
{
    ...
    internal class UIRefs
    {
        ...
        internal UIRefs(AssetBundle bundle) :
    ...
    internal static class UIFactory
    {
        ...
        internal static GameObject Window(this GameObject go, UIRefs refs)
        {
            ...
            go.Wrap(new GameObject("ClothesControl")).Control(refs, ClothesControl);
            go.Wrap(new GameObject("AccessoryControl")).Control(refs, AccessoryControl);
            return go;
        }
        ...
        // create each {label, button, state} control component
        internal static GameObject Control<TLabels, TStatus>(this GameObject go, UIRefs refs, StateControl<TLabels, TStatus> control)
        {
            go.AddComponent<RectTransform>();
            ...
            control.ComposeControl(
                go.Wrap(new GameObject("Labels")).VerticalLayout(control),
                go.Wrap(new GameObject("Buttons").VerticalLayout(control)),
                go.Wrap(new GameObject("Status").VerticalLayout(control)), refs);
            return go;
        }
        internal static GameObject VerticalLayout<TLabels, TStatus>(this GameObject go, StateControl<TLabels, TStatus> control)
        {
            go.AddComponent<RectTransform>();
            go.AddComponent<LayoutElement>().preferredHeight = 25 * control.Labels.Count();
            go.AddComponent<VerticalLayoutGroup>().With(ui =>
            {
                ui.spacing = 1;
                ...
                if (Path.GetFileName(bundle.name).Equals("ui.unity3d"))
                {
                    disposableComponent.transform.Find("Window").Find("StateWindow")
                        .Find("Pose").Find("PosePtn").Find("Layout").With(layout =>
                        {
                            ...
                            layout.gameObject.Wrap(new GameObject("ShowHide")).UI(new UIRefs(bundle));
                        });
                    return;
                }
            }
